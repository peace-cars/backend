...
export class PermissionsService {
  ...
  async canAccessTradeIn(userId: string, userRole: Role, leadId: string): Promise<boolean> {
    ...
    if (userRole === Role.DISTRICT_MANAGER) {
      // DMs can access if they manage the specific branch directly
      if (userBranchId && leadBranchId === userBranchId) {
        return true;
      }
      
      // DMs can access if they manage the district the branch belongs to
      if (userDistrictId && leadBranchId) {
        const { data: branch } = await client.from('branches').select('district_id').eq('id', leadBranchId).single();
